...
namespace EZBudget.Queries
{
    public static class EZBudgetDB
    {
        ...
        public static List<PastMonth> GetPastMonthsExpenses_SpecificCategory_WithRange(int userID, int nbOfMonths, int globalCategoryId)
        {
            try
            {
                var minDate = DateTime.Now.AddMonths(-nbOfMonths);
                ...
                var globalCategory = (from user in context.Users
                                        where user.UserID == userID
                                        select user).FirstOrDefault().Budgets.FirstOrDefault()
                                              .Category_Global.FirstOrDefault(cat => cat.CategoryGlobalID == globalCategoryId);

                var pastMonthsValues = globalCategory
                    .Category_Monthly
                    .Select(x => x.Expenses)
                    .SelectMany(y => y).Where(x => x.CreationDate >= minDate && x.CreationDate < maxDate)
                    .GroupBy(
